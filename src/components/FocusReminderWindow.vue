<template>
  <NativeReminderWindowShell v-slot="{ startWindowDrag }" class="focus-reminder-window" @dismiss="dismiss">
...
async function loadReminder() {
  ...
  await nextTick()
  const disposition = await markFocusReminderReady(payload.revision)
  if (disposition !== 'shown') return
  presentReminder(payload.revision)
}

function presentReminder(revision) {
  if (reminder.value?.revision !== revision || presentedRevision === revision) return
  presentedRevision = revision
  primaryAction.value?.focus()
  if (reminder.value.soundEnabled) playSoundPreview('complete')
}
...
onMounted(async () => {
  ...
  unlistenPresented = await listen('focus-reminder:presented', event => {
    presentReminder(event.payload?.revision)
  })
  await loadReminder()
})
