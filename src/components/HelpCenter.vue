...
const documents = [
  ...
  },
  {
    ...
    sections: [
      { title: '近期完善（v0.4.2 范围内）', paragraphs: ['自 v0.4.2 发布以来，以下改进已合入主干、随下一个发布版本一起整理：'], items: [
        '桌面专注控制器多形态：在"设置 → 专注与休息"中可切换"轨道表盘""专注岛""经典卡片"三种桌面小窗形态，新增"控制器保持在最前面"独立开关',
        '专注与休息设置：新增独立的设置分组，番茄轮次（2–8 轮）、短休息（3/5/10/15 分钟）、长休息（10/15/20/30 分钟）和"自动开始休息"全部可调',
        '轨道表盘精修：232px 紧凑矢量表盘，去掉 ±5 角标，暂停按钮顶对齐 dock，辅助文字（状态、副标题、剩余）字号统一上调一档',
        '节律提醒随机文案：每条节律的标题与正文都支持一句随机提示语，避免重复感；后台到点通过系统通知中心送达',
        '节律提示窗：新增独立的桌面提醒窗，前后端任务可同时处理；提供延后、跳过、完成等处置',
        '专注工作台持续打磨：今日花朵区域同时显示花种名称与所处阶段；任务选择器重构后支持搜索 / 分组 / 颜色识别，并明确"开始前可调整 / 成长后可修正一次"的目标规则',
        '退出时不再驻留托盘：直接退出后系统托盘图标会同步清理',
        '重塑「个人空间 → 空间管理」：新增"应用已占用"总览，按任务数据、已用图片、已用文件、待处理附件、清理站、个人资料、本机恢复点等分类展示',
        '附件维护浏览器升级：待处理 / 清理站双 Tab 切换，每页 30 项分页，图片可直接预览；永久删除前必须二次确认',
        '失效引用提示：扫描发现任务或备注指向不存在的文件时只提示，不修改原任务',
        '富文本编辑器大幅扩充：新增撤销 / 重做、下划线、高亮（多色）、行内代码、代码块、表格、折叠块、任务引用、居中 / 左对齐和字符 · 词数 · 阅读时间状态栏；粘贴网址时自动识别为链接',
        '分组图标选择器重做：覆盖工作、自然、娱乐、动物、饮食、交通、健康、居家、财务、沟通等主题，顶部搜索框支持中文关键词',
        '任务日期浮层重设计：时间、提醒、重复三个分组默认折叠，时间区域提供 08:00、09:00、10:00、12:00、14:00、18:00、20:00 快捷预设；自定义支持输入 930 或 09:30，并能用减号 / 加号按 15 分钟微调',
        '清理站分页、确认弹层和图片预览体验统一收紧，避免误删'
      ], tip: '这些改动都已合入主干，会随下一个发布版本一起对外整理。' },
      { title: 'v0.4.2 · 核心发布', paragraphs: ['本版本聚焦专注成长、花田成就和桌面端体验：'], items: [
        ...
        '后台到期时显示原生置顶提醒窗；专注可打开轨道表盘、专注岛或经典卡片桌面控制器',
        '新增专注与节律回顾：可查看趋势、筛选记录、展开详情和管理历史',
        '持续完善本机恢复、附件维护、通知权限与 Windows/macOS 的桌面体验'
      ] },
