...
const documents = [
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '收集箱、完成记录与垃圾桶', paragraphs: ['收集箱用于临时记录尚未整理的事项；完成记录用于跨清单回顾已完成的进展；误删任务会先进入垃圾桶，方便恢复。'], image: { src: '/screenshots/inbox-view.webp', alt: '收集箱视图', caption: '收集箱顶部出现“整理收集箱”流，可直接把未安排事项加入今日或安排到明天' }, items: [
        '收集箱：临时存放还没分类的任务；顶部会提示待决定项，可逐项加入今日、安排到明天，或打开详情继续整理',
        '完成记录：跨清单汇总已完成任务，可按完成时间、清单、近 7 天 / 30 天 / 全部及关键词筛选；分组可折叠，超过 50 项会分页加载',
        '垃圾桶：恢复误删任务，或将其永久删除'
      ] },
  ...
  },
  {
    ...
    sections: [
      { title: 'v0.5.1 · 花田回顾与更新体验', paragraphs: ['这次更新继续打磨任务推进体验，并把专注花田从“当天的花”扩展成可长期回顾、收藏和解锁的成长空间：'], items: [
        '专注成就重构为成长总览、花田回顾、花种图鉴和成长徽章四个页面；年度花田与右侧年度档案、近期足迹对齐，浏览信息更集中',
        '年度档案突出本年度投入、完整盛放次数、常种花与最丰盛月份；近期足迹保留下一花种和最近解锁的成长徽章',
        '点击花田中的日期可查看更完整的当天花详情：目标、完成进度、有效专注总时长以及按任务归类的起止时间线',
        '花种图鉴新增解锁门槛与剩余分钟提示；已解锁花种保留门槛，未解锁花种同时展示还差多少分钟，后期花种的解锁梯度更有挑战',
        '任务工作流和桌面体验继续优化：今日、收集箱、计划、完成记录、任务详情、个人空间、主题和节律控制器的层级更清晰稳定',
