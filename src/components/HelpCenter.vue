...
const themePalettes = {
  ...
  amber: { accent: '#b97822', accentStrong: '#8d5814', accentSoft: '#fff1dc', accentTint: '#fffaf3' },
  coral: { accent: '#c85e63', accentStrong: '#9f4148', accentSoft: '#fdebed', accentTint: '#fff8f8' }
}
...
const documents = [
  {
    ...
    sections: [
      ...
      { title: '3. 完成与回顾', paragraphs: ['完成后勾选任务；记录会保留在"完成记录"里，可按完成时间、清单、近 7 天 / 30 天 / 全部或关键词回顾，必要时还能直接恢复。'], tip: '每天结束前花一分钟整理收集箱，第二天会更轻松。' }
    ]
  },
  {
    ...
    sections: [
      { title: '你可以管理什么', paragraphs: ['易简清单把日常安排拆成几类对象。先分清它们的作用，再决定任务放在哪里：'], image: { src: '/screenshots/sidebar-overview.webp', alt: '应用功能入口总览', caption: '模块栏与清单侧栏共同放置应用入口、视图、清单和常用操作' }, items: [
        ...
        '视图：从不同角度查看同一批任务，例如今日、收集箱、计划、重要、完成记录和垃圾桶',
      ...
      { title: '常用入口在哪里', paragraphs: ['桌面端打开后，最左侧是全局模块栏，清单栏、任务列表和任务详情依次展开。没有选中任务时，右侧会显示空状态；这不是额外的任务栏。'], image: { src: '/screenshots/sidebar-overview.webp', alt: '应用三栏布局', caption: '三栏布局：左侧模块栏 + 清单侧栏、中间任务列表、右侧详情/空状态' }, items: [
        ...
        '清单栏右上角：收起清单栏。收起后，模块栏会接管搜索、主要视图、清单切换、完成记录和垃圾桶等快捷操作',
        '左侧主要视图：今日、收集箱、计划、重要；"完成记录"独立成行，紧跟主要视图之后',
      ...
      { title: '当前不包含的功能', paragraphs: ['当前版本是本地个人任务管理工具：'], items: [
        ...
        '不包含完整日历、习惯或四象限管理'
      ], tip: '个人空间只展示当前可用的本机资料、存储维护与恢复能力。' }
    ]
  },
  {
    ...
    sections: [
      ...
      { title: '任务操作', paragraphs: ['任务支持多种快捷操作：'], image: { src: '/screenshots/task-detail-meta.webp', alt: '任务详情顶部属性行', caption: '任务详情顶部属性行展示日期、优先级、清单等可操作的元数据' }, items: [
        ...
        '完成：点击复选框标记完成，支持撤销',
        '删除：移入垃圾桶，可设置自动清理天数'
      ], tip: '已完成的任务会保留记录，方便在“完成记录”中按清单或时间回顾，必要时可以直接恢复。' },
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '视图说明', paragraphs: ['应用提供多种视图帮助你管理任务，主要视图位于清单侧栏顶部，完成记录独立成行：'], image: { src: '/screenshots/today-view.webp', alt: '今日视图', caption: '今日视图聚焦当天需要推进的任务' }, items: [
        '今日：聚焦今天需要推进的任务，包括手动加入和今日到期的任务',
        '收集箱：临时存放未分类的任务，可在顶部整理流中直接安排日期或加入今日',
        '计划：按日期分组显示所有有日期的任务，默认按周前后切换，按天承接已安排任务',
        '重要：汇总所有高优先级、置顶和标记重要的任务',
        '完成记录：跨清单回顾已完成任务，可按完成时间、清单、近 7 天 / 30 天 / 全部或关键词筛选；任务仍可直接恢复',
      ...
      { title: '收集箱：先记下，再决定', paragraphs: ['收集箱用于临时记录尚未整理的事项。顶部的“整理收集箱”会显示待决定项；可直接加入今日、安排到明天，或打开详情继续补充清单、日期和标签。'], image: { src: '/screenshots/inbox-view.webp', alt: '收集箱整理流', caption: '收集箱把待决定事项集中展示，可从卡片直接加入今日或安排到明天' }, tip: '不确定归属时先放进收集箱，避免为了分类而中断记录。' },
      { title: '完成记录：按条件回顾进展', paragraphs: ['勾选完成的任务不会消失，而是进入“完成记录”。这里跨清单汇总完成事项，并保留恢复入口。'], image: { src: '/screenshots/completed-view.webp', alt: '完成记录筛选与分组', caption: '完成记录支持关键词、清单和时间范围筛选，并按完成时间分组' }, items: [
        '用关键词搜索任务标题、标签或备注内容',
        '按清单及近 7 天、近 30 天或全部完成时间范围筛选',
        '展开或折叠日期分组；超过 50 项时按需加载更多',
        '打开任务详情或直接恢复为未完成'
      ] },
      { title: '垃圾桶：恢复或永久删除', paragraphs: ['删除任务或清单后，它们会先进入垃圾桶。可恢复误删项目；确认无误后再永久删除。'], image: { src: '/screenshots/trash-view.webp', alt: '垃圾桶中的任务和清单', caption: '垃圾桶分别列出已删除清单与任务，并提供恢复和永久删除操作' }, tip: '删除清单时，其中任务会一同进入垃圾桶；永久删除后无法恢复。保留时长可在“个人空间 → 存储与清理”调整。' },
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '桌面专注控制器：三种形态随你选', paragraphs: ['在"专注工作台"点击"打开桌面控制器"或专注进行中顶部的"显示桌面控制器"按钮，可以弹出一个独立的桌面小窗，让你在切换到其他应用时仍能看到进度、调整时间和完成本轮。'], image: { src: '/screenshots/settings-focus.webp', alt: '桌面专注控制器形态设置', caption: '在"设置 → 专注与节律"中可以切换桌面控制器的形态，并控制是否保持在最前面' }, items: [
        ...
        '切换方式：打开"设置 → 专注与节律"，在"桌面专注控制器"中点选想要的形态卡片，下次开始专注时生效',
      ...
      { title: '专注回顾看清真实投入', paragraphs: ['"时钟 → 专注回顾"汇总专注和节律两类记录，分"概览"和"管理"两个二级页面：概览看亮点和趋势，管理看明细和处置。'], image: { src: '/screenshots/focus-history.webp', alt: '专注回顾', caption: '专注回顾默认进入概览页：4 个核心指标 + ECharts 趋势图 + 节律响应与时段分布' }, items: [
        '概览页：本期亮点 + 4 个核心指标（专注投入 / 完成率 / 暂停次数 / 节律次数）+ ECharts 趋势图 + 节律响应速度 / 时段分布 / 工作日-周末对比 + 最近发生',
        '管理页：搜索 / 范围 / 细分三段式筛选条件，可按专注方式、结果、暂停情况、排序等多维筛选；每页 30 条分页',
        '批量操作：管理页支持多选 + 批量删除（带撤销）；筛选偏好会持久化，下次自动恢复',
        '导出能力：概览页支持 Markdown 报告 / HTML 文件 / CSV 文件，原生保存对话框按类型过滤',
        '时间范围：顶部切换今日 / 本周 / 本月 / 全部，或自定义日期 popover；所有统计和列表同步使用同一时间窗口',
        '趋势对比：4 个核心指标都会与上一周期（上周 / 上月 / 去年）做同口径对比，正负变化直接显示',
        '节律分桶可调：响应速度分布的分桶粒度可调；工作日 / 周末对比展示节律到点分布差异',
        '专注方式快照：编辑记录时原专注方式会被一起保存，避免回看时找不到对应 profile',
        '详情面板：点击任意记录在右侧打开详情，可编辑备注、调整结果分类；专注详情展示过程摘要与关联任务'
      ], tip: '专注回顾和专注成就使用同一份数据，但回顾聚焦"什么时候做了什么"，成就聚焦"留下了什么"。' },
      { title: '专注成就让长期投入可见', paragraphs: ['"时钟 → 专注成就"是独立的长期反馈页面，与工作台、节律、回顾平级。它回答"这些投入留下了什么"，不进入开始专注前的决策。'], image: { src: '/screenshots/focus-achievement.webp', alt: '专注成就', caption: '专注成就默认进入花田总览：年 / 月格温室 + 累计成长环形图 + 近期足迹' }, items: [
        '花田总览：默认页，先展示当前年份的十二格温室（年 / 月聚合，玻璃罩+木质底座+山丘风景），再展示本月花圃（每天一株）',
        '累计成长 + 近期足迹：累计成长用环形图展示（距离下一等级还差多少分钟），下方"近期足迹"列出最近解锁的成长徽章和下一花种预览',
        'Drill down：点击年格展开到月格，点击月格展开到每天，每天的花种、阶段、当日成长分钟都可查',
        '花种图鉴：互动植物舞台 + 六阶段成长回放 + 主题花圃收藏。已解锁、下一可解锁、未解锁花种在同一页浏览',
        '第一册 12 种花按主题：晨光（小雏菊 / 郁金香 / 波斯菊 / 向日葵）、微风（虞美人 / 薰衣草 / 鸢尾花 / 百合）、暮色（绣球花 / 山茶花 / 牡丹 / 月光花）',
        '成长徽章：按"开始 / 积累 / 深入 / 多样"四类组织；首次盛放、累计盛放、累计有效专注、培养多花种等非连续里程碑',
        '历史收获：旧的蓝莓 / 草莓 / 番茄 / 西瓜 / 南瓜等果实奖励只读保留，不继续产生',
        '解锁机制：花种按累计有效专注分钟（0、150、450、900、1500、2250、3300、4800、6600、8700、11400、15000）逐步解锁；首次跨过门槛会播放收藏印章演出',
        '陪伴等级：已解锁花种会按该花累计培养分钟与盛放次数，从初识、初绽逐步成长到相伴、熟稔和共生；第一册集齐后仍可继续深入培养喜欢的花'
      ...
      { title: '成长数据如何计算', paragraphs: ['花田从升级到当前版本后开始记录，不把过去的果实奖励自动折算为花朵。新完成记录不再生成果实，旧记录中的果实仍可在专注成就里查看。'], items: [
        '完成或提前结束的专注阶段，都会按实际整分钟计入成长；系统中断不计入',
  ...
  {
    id: 'settings', category: '设置与数据', title: '个性化设置', summary: '调整外观、任务显示、专注节奏、通用行为和通知声音。', icon: Settings,
    sections: [
      { title: '外观与显示', paragraphs: ['在设置中可以统一调整应用外观、面板、任务展示方式和已完成任务在列表中的呈现：'], image: { src: '/screenshots/settings-appearance.webp', alt: '外观与显示设置', caption: '外观与显示支持主题、密度、面板和任务展示方式' }, items: [
        '配色主题：青绿、海蓝、紫罗兰、石墨四种主题',
        '主题背景：让侧栏、任务区和详情区使用当前主题的低饱和背景',
        '信息密度：舒适模式和紧凑模式',
        '界面显示：默认展开任务侧栏、默认展开任务详情面板、详情栏宽度都可单独控制',
        '任务展示：列表模式显示已完成任务、分组模式下保留已完成任务、默认显示已完成任务、显示完成用时——这些选项只影响展示，不改变任务数据'
      ], tip: '设置面板底部会显示"已自动保存/正在保存/保存失败"状态；本轮改动一般几秒内就会落盘。' },
      { title: '专注与节律', paragraphs: ['"专注与节律"集中管理桌面控制器、番茄节奏和休息安排。'], image: { src: '/screenshots/settings-focus.webp', alt: '专注与节律设置', caption: '专注与节律设置中可切换桌面控制器形态、调整番茄轮次和休息时长' }, items: [
      ...
      { title: '通用', paragraphs: ['"通用"决定打开应用后的落点、启动提示风格和关闭窗口时的行为。'], image: { src: '/screenshots/settings-behavior.webp', alt: '通用设置', caption: '通用设置支持默认启动视图、启动提示和关闭窗口行为' }, items: [
        '默认启动视图：今日、收集箱、计划或重要',
        '启动提示：开启后根据任务状态显示本地提示，可选择轻松、务实或鼓励风格',
        '关闭窗口：选择最小化到后台或直接退出应用；最小化后可从 Windows 通知区域或 macOS Dock/菜单栏恢复'
      ] },
      { title: '通知与声音', paragraphs: ['"通知与声音"把到期提醒和操作音效分开管理：'], image: { src: '/screenshots/settings-notifications.webp', alt: '通知与声音设置', caption: '通知与声音支持测试系统提醒，并按类别开关操作音效' }, items: [
  ...
  },
  {
    ...
    sections: [
      { title: '数据维护入口', paragraphs: ['数据维护集中在“个人空间”中，而不是设置面板：'], image: { src: '/screenshots/profile-space.webp', alt: '个人空间主面板', caption: '个人空间主面板：昵称、头像和数据维护的总入口' }, items: [
        '个人空间 → 存储与清理：扫描本机应用数据，按任务数据、已用图片、已用文件、待处理附件、清理站、个人资料、本机恢复点等分类显示；可一键把未引用附件移入清理站，并在双 Tab 浏览器中分页浏览、预览和永久删除',
        '清理站：查看、恢复或永久删除附件；任务和回收站中的引用会保留；永久删除前必须二次确认',
        '个人空间 → 备份与恢复：创建、打开、恢复和删除本机恢复点'
      ...
      { title: '个人资料与头像', paragraphs: ['点击左侧顶部的头像或昵称，打开个人空间 → 概览。这里可以修改本机显示的昵称和头像。'], image: { src: '/screenshots/avatar-picker.webp', alt: '头像选择器', caption: '头像选择器提供内置头像，也支持上传本地头像' }, steps: [
        '点击左侧顶部头像，进入个人空间。',
        '在概览页面点击头像，打开头像选择器。',
      ...
      { title: '本机存储与清理', paragraphs: ['"个人空间 → 存储与清理"按需扫描本机数据，把应用占用的每一类文件分开统计，并提供安全的附件整理流程。'], image: { src: '/screenshots/space-management.webp', alt: '存储与清理', caption: '存储与清理可扫描本机附件、查看历史迁移备份和设置垃圾桶保留时间' }, items: [
        '应用总占用：扫描后顶部会显示应用整体占用的存储空间，分类采用互斥口径，不会重复相加',
        '空间构成明细：可展开查看任务数据、已用图片、已用文件、待处理附件、清理站、个人资料、本机恢复点、历史迁移备份、其他应用数据等分类大小',
        '状态摘要：根据当前情况显示"可安全整理 X / 可释放 Y MB"或"清理站待处理 N 项"，并指出下一步建议',
        '待处理附件：未被任何任务、备注或回收站引用的附件；支持一键移入清理站',
        '清理站：暂存区内的文件仍可一键恢复；永久删除前必须二次确认',
        '本机恢复点：单独占一栏展示当前所有恢复点总大小，可一键跳到"备份与恢复"管理',
        '历史迁移备份：应用升级前自动保留的兼容性副本；点击"查看文件"打开迁移备份查看器，永久删除前必须二次确认',
      ...
      { title: '本机恢复点', paragraphs: ['打开个人空间 → 备份与恢复，可以为当前设备创建恢复点。恢复点包含任务、附件和头像，适合在大批量整理或安装更新前使用。'], image: { src: '/screenshots/profile-security.webp', alt: '备份与恢复', caption: '备份与恢复页面可创建、打开、恢复和删除本机恢复点' }, steps: [
        '点击“创建恢复点”，等待本机备份完成。',
  ...
  {
    id: 'updates', category: '更新说明', title: '检查与安装更新', summary: '在设置中查看版本、更新说明并安装可用更新。', icon: Wrench,
    sections: [
      { title: '在应用内更新', paragraphs: ['打开“设置 → 关于与更新”，可查看当前版本、本次更新亮点和历史版本说明。应用启动后会静默检查更新；发现新版本时，设置入口会出现提示。'], items: [
        '点击“检查更新”可手动重新检查；开发环境不会请求更新服务',
        '发现可用版本后，先阅读更新说明，再点击“下载并安装”；下载期间请保持应用打开',
        '下载完成后会自动校验更新包签名；显示“更新已安装”时点击“立即重新启动”即可进入新版本',
        '可以跳过某个版本；发布更高版本后，或手动重新检查时会再次提示',
        '更新源可选择自动（自建服务器优先）、自建服务器或 GitHub；网络异常不会影响本机任务数据'
      ], tip: '更新前若要进行大批量整理，建议先创建本机恢复点；更新不会主动上传任务内容。' },
      { title: 'v0.5.3 · 专注工作流与花种图鉴优化', paragraphs: ['本次更新围绕专注循环与花种图鉴的连续体验展开，让任务不因休息被打断，成长信息也更清晰紧凑：'], items: [
        '专注工作台的更换任务选择器补充最近使用任务；专注回顾标签栏在滚动时顶部吸附，切换页面更方便',
        '任务详情中的专注投入默认仅展示最近三段，支持展开、收起及进入完整回顾；无论是否已有记录都可直接开始专注',
        '跳过休息或完成休息后，下一轮会默认延续当前关联任务，不再要求重复选择',
        '花种图鉴采用按花种适配的水彩场景与专属插画，重新梳理花语、档案、陪伴成长、进度节点和操作区的紧凑布局',
        '修复 macOS 上悬停查看今日花大图时浮层背景偶发透明的问题'
      ] },
      { title: 'v0.5.2 · 任务详情与成长体验优化', paragraphs: ['本次更新重点打磨任务详情、专注操作和成长回顾中的细节，让信息切换和持续追踪更顺畅：'], items: [
        '“设置 → 外观与显示”新增任务详情展开方式：可选择在当前窗口内展开，或在打开详情时自动扩展窗口，并可在设置中预览效果',
        '修复从时钟切回主界面后任务详情区域残留的问题；侧栏收起时的时钟布局同步优化',
        '专注开始前的任务选择器改为紧凑浮层，关联任务、搜索结果和计划信息更清晰；专注回顾筛选也改为更易收起和浏览的浮层',
        '成长徽章补充收藏卡、详情和追踪信息；花种图鉴持续优化解锁门槛、培养进度、花语、回放轨道与陪伴等级展示',
        '修复 macOS 更新完成后偶发停留在“正在重启”的问题，安装完成后会恢复正常重启流程'
      ] },
      { title: 'v0.5.1 · 花田回顾与更新体验', paragraphs: ['专注成就扩展为可长期回顾、收藏和解锁的成长空间，并继续打磨任务与更新体验：'], items: [
        '专注成就重构为成长总览、花田回顾、花种图鉴和成长徽章四个页面；当天花详情可查看目标、完成进度、有效专注总时长及按任务归类的时间线',
        '花种图鉴新增解锁门槛与剩余分钟提示；任务工作流、个人空间、主题和节律控制器的层级更清晰稳定',
        '“设置 → 关于与更新”改进更新说明展示：远端 Release Notes 会保留标题、正文和列表层级，更新源选择框不再遮挡文字'
      ] },
      { title: 'v0.4.7 · 清单工作流与节律控制器', paragraphs: ['这次更新把任务从收集、安排到完成回顾的几个关键页面重新梳理，并让节律提醒在桌面上更容易持续跟进：'], items: [
        '今日视图重组为“今日计划、今天到期、建议加入今日”三层，不再把所有任务混在同一个列表里',
        '收集箱新增整理流：未安排事项可直接加入今日、安排到明天，或打开详情继续整理',
        '计划改为可前后切换的周视图，按天承接已安排任务；逾期和周外任务仍会显示，不会被周视图隐藏',
        '已完成改为跨清单完成记录：支持按完成时间、清单、近 7 天 / 30 天 / 全部及关键词回顾；任务仍可直接恢复',
        '任务详情新增专注投入闭环：无投入时可从任务直接开始专注；已有记录可查看每段投入并进入完整回顾',
        '节律桌面控制器默认停靠屏幕右上角：紧凑态显示当前节律与计时，展开后按最多三项提醒展示对应操作，可再次收起，窗口高度随实际项数调整',
        '任务完成状态与详情主题卡片优化：已完成任务保持可读，日期与关联信息按状态收束，减少重复操作'
      ] },
      { title: 'v0.4.6 · 专注模块深度升级', paragraphs: ['这次更新把专注回顾、专注成就、专注详情都做了大改版，专注数据可以更深入地查看和回顾：'], items: [
        '专注回顾图表全面升级为 ECharts：折线/柱状支持悬停信息卡、长范围去拥挤、4 个核心指标周期对比',
        '专注回顾管理页：搜索/范围/细分三段式筛选条件，自绘下拉替换原生 select；时间范围支持今日/本周/本月/全部/自定义日期 popover',
        '导出能力扩展：专注回顾支持 Markdown 报告、HTML 文件、CSV 文件；保存对话框按类型过滤',
        '批量操作与偏好：管理页支持多选与批量删除（带撤销），筛选偏好会持久化',
        '节律分桶可调 + 工作日/周末对比：分桶粒度可调，可以对比工作日和周末的节律到点',
        '专注详情重构：详情面板头部更紧凑、统计并入时长卡、时间线和分区更清晰',
        '专注详情展示过程摘要与关联任务：打开记录可看到完整过程摘要，点击关联任务直接跳转',
        '专注方式快照：编辑记录时原专注方式会被一起保存，避免回看时找不到对应 profile',
        '专注成就温室化：年/月格改成温室风格（玻璃罩+木质底座+山丘风景），点击年格展开到月格、点击月格展开到每天',
        '累计成长加环形图 + 近期足迹：右上栏目合并成"成长仪表 / 近期足迹"两栏',
        '空间管理修复：备份统计修正，并支持管理迁移备份（双 Tab 浏览器分页浏览）',
        '「设置」入口可用更新红点：有新版本时设置入口显示红点提醒；更新源地址增加 releases 路径前缀，保留旧地址兼容',
        '三平台更新包：Windows、macOS Intel 与 Apple Silicon 均提供独立安装包和已签名自动更新包；修正 macOS 已是最新版时的错误提示'
      ] },
